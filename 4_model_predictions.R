...
# two-model approach ####
# Aggregate to get total crimes per week per ZIP code
...
# Prepare for merging back to the original dataset if needed for features
app_df <- left_join(app_df, monthly_crimes, by = c("zip", "month", "year"))

data_prep_regression <- recipe(total_crimes ~ month + year + zip, data = monthly_crimes) %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  prep(training = monthly_crimes, retain = TRUE)

prepared_data_regression <- bake(data_prep_regression, monthly_crimes)

# Splitting the dataset for regression
set.seed(3212024) # Ensure reproducibility
training_indices_regression <- createDataPartition(prepared_data_regression$total_crimes, p = 0.8, list = FALSE)
train_data_regression <- prepared_data_regression[training_indices_regression, ]
test_data_regression <- prepared_data_regression[-training_indices_regression, ]

rf_regression_model <- ranger(
  total_crimes ~ ., 
...
# Applying to models for specific predictions ####
...
# Train the Random Forest model with aggregated data
